...
def compute_conditional_log_prob(model, tokenizer, prompt, continuation,
    ...
    input_ids = tokenizer.encode(total_sequence, return_tensors="pt", add_special_tokens=False)
...
def main():
    ...
    dataset_name = '.'.join(args.dataset_path.split("/")[-4:-1])
    pbar = tqdm(enumerate(random.sample(range(len(dataset)), args.num_samples)), total=args.num_samples,
                desc="Model: {}, Dataset: {}".format(model_name, dataset_name))
    for idx, i in pbar:
        ...
        if 'answer_choices' in row.keys():
            ...
            raise Exception("Invalid answer choices")
        ...
        try:
            output = model.generate(input_ids, max_new_tokens=2*len(true_dec), pad_token_id=50256,
                                    do_sample=False, top_k=1, top_p=0.95, num_return_sequences=1, attention_mask=attention_mask)
        except:
    ...
    val_ds = '/'.join(args.dataset_path.split("/")[-4:-2])
    ...
    save_dir = os.path.join(args.model_name_or_path, "inference", 
                            val_ds, f"n_samples_{args.num_samples}")
    # check if dir exists
