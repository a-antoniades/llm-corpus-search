...
VALIDATION_DATASET='/share/edc/home/antonis/datasets/huggingface/flan_v1_task_ds_p_10'
DATASET_TYPE=$(echo "$DATASET" | awk -F/ '{print $(NF-1) "/" $NF}')
BATCH_SIZE=3
REPORT_TO=False
export CUDA_VISIBLE_DEVICES=0
python train_gpt_copy.py \
   --count_tokens False \
   --model_type pythia \
   --model_name_or_path "EleutherAI/pythia-200M-deduped" \
   --rand_init_weights True \
   --do_train \
   --output_dir "./models/pythia/scratch" \
   --fp16 True \
   --num_train_epochs 1 \
   --per_device_train_batch_size $BATCH_SIZE \
   --gradient_checkpointing \
...
train_gpt.py \
   ...
   --do_train \
   --do_eval \
   --output_dir "./models/pythia/experiment_1" \
   --fp16 True \
   --num_train_epochs 1 \
   --per_device_train_batch_size $BATCH_SIZE \
   --gradient_checkpointing \
